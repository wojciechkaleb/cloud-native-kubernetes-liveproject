...
class HealthController {
    ...
    liveness(req, res) {
        try {
            ...
            this.logger.info('Liveness probe check passed')
    ...
    async readiness(req, res) {
        try {
            ...
            if (allChecksPass) {
                this.logger.info('Readiness probe check passed', { checks })
                res.status(200).json(healthStatus)
            } else {
    ...
    async health(req, res) {
        try {
            ...
            this.logger.info(`Health check completed with status: ${healthStatus.status}`, { checks })
